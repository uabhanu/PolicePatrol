...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		{	
			...
			Debug.Log("Files Mod: "+mod.files.ToString());
			foreach( string filePath in mod.files ) {
				string absoluteFilePath = System.IO.Path.Combine( mod.path, filePath );
				Debug.Log("FilePath: "+absoluteFilePath);
				this.AddFile( absoluteFilePath, modGroup );
			}
			
			Debug.Log( "Adding folders..." );
			Debug.Log("Files Mod: "+mod.folders.ToString());
			foreach( string folderPath in mod.folders ) {
				string absoluteFolderPath = System.IO.Path.Combine( Application.dataPath, folderPath );
				Debug.Log("FilePath: "+absoluteFolderPath);
